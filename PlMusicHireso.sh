...
if [ "${ext4}" = ".m4a" ]; then
  ...
  AUDIODRIVER=alsa play /run/tmp.wav -r 192k -b 24 --buffer 81920 $effect &
elif [ "${ext4}" = ".mp4" ]; then
  yes | ffmpeg -i "$1" /run/tmp.wav
  echo "Upsampling : 192k 24bit"
  AUDIODRIVER=alsa play /run/tmp.wav -r 192k -b 24 --buffer 81920 $effect &
  ...
  echo "Upsampling : 192k 24bit"
  AUDIODRIVER=alsa play "$1" -r 192k -b 24 --buffer 81920 $effect &
elif [ "${ext4}" = ".mp3" ]; then
  ...
  if [ $rate2 -lt 50000 ]; then
    ...
    AUDIODRIVER=alsa play "$1" --buffer 81920 $effect &
  fi
elif [ "${ext4}" = "flac" ]; then
